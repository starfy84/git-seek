...
use trustfall::{Schema, provider::{Adapter, resolve_coercion_using_schema}};
...
static SCHEMA: LazyLock<Schema> = LazyLock::new(|| {
    Schema::parse(include_str!("schema.graphql")).expect("schema not valid")
});
        ...
        match edge_name.as_ref() {
            "repository" => {
                let repo_name = match self.git2_repo.find_remote("origin") {
                    Ok(remote) => {
                        remote.url().and_then(|url| {
                            url.trim_end_matches(".git").rsplit('/').next().map(|s| s.to_string())
                        })
                    }
                    Err(_) => None
                }.unwrap_or_else(|| {
                ...
                Box::new(std::iter::once(Vertex::Repository(Repository::new(repo_name))))
            }
