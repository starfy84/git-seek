...
/// Run the CLI with the given parsed arguments and a specific repo path.
pub fn run_with_repo(cli: Cli, repo_path: &std::path::Path) -> anyhow::Result<()> {
...
/// Run the CLI using the repository from the current environment.
